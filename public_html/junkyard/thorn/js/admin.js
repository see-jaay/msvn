...
function AdminUpdate(dt) {
	// ED1_34HDT();
	// console.log(editor);
	if(editor && editor.isInit)
		editor.Update(dt);

}

// $('.adm-button').addEventListener('click', (e)=>{ $(this).toggleClass('active'); });

document.addEventListener('mousedown', (e)=>{

	if(e.path[0].classList.contains('adm-button') ){
		var elm = e.path[0];

	 	if( elm.classList.contains('switch')) {
		 	$('.adm-button.switch.active').removeClass('active');
		 	elm.classList.add('active');
	 	}
	 	else if(elm.classList.contains('once')){
		 	elm.remove();
	 	}
	}
});
...
function EditMode() {
	// evt_editor.dispatch("toggleEM")
	$('#site_wrapper').toggleClass('edit-mode');
	$('#edtr_wrapper').toggleClass('edit-mode');


	let ep = $('#editor_panel_secondary_wrapper .inner');
	if(isEditMode){
