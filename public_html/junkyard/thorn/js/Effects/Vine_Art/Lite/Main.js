...
class VineLocal {
	...
	Init(){
		var c = '<canvas id="VC--'+ this.index +'" class="vine-canvas lyr def"></canvas>';
		var va = $('#vineArt');
		va.append(c);
		console.log(va);
	...
	Set()
	{
		...
		if(this.depth)
		{
			...
			// this.canvas.style.filter = 'blur('+(this.blur * .8)+'px)';
		}
	}
...
//	thorn shape
		...
		for(var i = 0; i < thorns.length; i++)
		{
			...
			if(x > x1 && x < x2 && y > y1 && y < y2)
			{
				...
				ctx.moveTo(0 + 7.5,0);
				ctx.lineTo(9 + 7.5, 13);
				ctx.lineTo(20 + 7.5, 19);
				ctx.lineTo(36 + 7.5, 16);
				ctx.lineTo(40 + 7.5, 20);
				ctx.lineTo(40 + 7.5, 26);
				ctx.lineTo(24 + 7.5, 46);
				ctx.lineTo(10 + 7.5, 80);
				ctx.lineTo(0 + 7.5, 120);
...
function InitializeVines() {
	...
	while(va.css('--v' + vin))
	{
		var css = va.css('--v'+vin).replace(/\s/g, '').split(',');
		var vine = new VineLocal(vin, css[0], css[1], parseFloat(css[2]), 1.3 + parseFloat(css[3]), parseFloat(css[4]));
